...
const ViewGamcaSlip = ({ auth, gamcaSlips = [] }) => {
    ...
    useEffect(() => {
        ...
        if (searchTerm.trim() !== "") {
            ...
            data = data.filter((item) => {
                ...
                const gamcaSlipId = item.id?.toLowerCase() || "";
        ...
        setCurrentPage(1);
    }, [searchTerm, fromDate, toDate, gamcaSlips]);
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: "Gamca Slip List",
    });

    return (
        <AdminDashboardLayout
            user={auth.user}
            header={<h1 className="font-semibold text-xl text-gray-800">Gamca Slip List</h1>}
        >
            <Head title="Gamca Slip List" />
            ...
            <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-8">
                ...
                <div className="bg-white shadow-sm rounded-lg p-4 flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-end gap-3 mb-4">
                    <div className="flex flex-col">
                        ...
                        <input
                            type="text"
                            placeholder="ID/Name / Passport/Agency"
                            className="border rounded-lg px-3 py-2 w-full sm:w-52 md:w-60 text-sm"
                            value={searchTerm}
                            onChange={(e) => {
                                let value = e.target.value;
                                value = value.replace(/[\u200B-\u200D\uFEFF]/g, ""); // invisible spaces
                                value = value.replace(/\s+/g, " ").trim(); // normal cleanup
                                setSearchTerm(value);
                            }}
                        />
                    ...
                    <table className="min-w-full text-sm text-left text-gray-700 border-collapse">
                        <thead className="bg-gray-100 text-gray-800">
                            <tr>
                                ...
                                <th className="px-3 py-2">Traveling Country</th>
                            ...
                            {paginatedData.length ? (
                                paginatedData.map((item, i) => (
                                    <tr key={item.id} className="hover:bg-gray-50 border-b last:border-0">
                                        ...
                                        <td className="px-3 py-2 font-medium">{item.passport_image && (
                                            <img
                                            src={`/uploads/passports/${item.passport_image}`}
                                            alt="Pre Medical"
                                            className="w-16 h-16 object-cover rounded-lg border"
                                            />
                                        )}</td>
                                        <td className="px-3 py-2">{item.travel_country}</td>
                                        <td className="px-3 py-2">
